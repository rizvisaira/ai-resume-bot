...
load_dotenv()
os.getenv("PUSHOVER_API_KEY")  # Ensure the API key is loaded
...
class AgentTools:
    ...
    def record_user_details(self, email, name="Name not provided", notes="not provided"):
        self.__push(f"Recording {name} with email {email} and notes {notes}")
        return {"recorded": "ok"}

    def record_unknown_question(self, question):
        self.__push(f"Recording {question}")
        return {"recorded": "ok"}